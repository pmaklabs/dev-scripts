...
# Usage: compare-saml-meta-to-jks.sh <SAML_METADATA_XML|-> <KEYSTORE.jks|.p12>
set -Eeuo pipefail
...
Pass a path to the metadata XML file, or '-' to read XML from STDIN.

Requires: openssl, keytool (from a JRE/JDK). perl is optional.
...
echo "==[1/5] Loading metadata XML..."
META_XML="$WORKDIR/meta.xml"
if [[ "$META_SRC" == "-" ]]; then
  cat > "$META_XML"
else
  [[ -f "$META_SRC" ]] || { echo "ERROR: metadata XML not found: $META_SRC" >&2; exit 2; }
  cp "$META_SRC" "$META_XML"
fi
...
# Write unique PEMs + (serial, sha256)
i=0
while IFS= read -r b64; do
  ...
  fp="$(sha256fp "$pem")"
  # de-dup by fingerprint primarily (exact identity of cert)
